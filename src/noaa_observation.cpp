//noaa_observation.cpp

#include "observation_implementation.h"
using namespace obs;

/* ----------------------------------------------------------------------------
NOAA Observation Member Definitions
---------------------------------------------------------------------------- */

NOAAObservation::NOAAObservation(){};
NOAAObservation::~NOAAObservation(){};

void
NOAAObservation::load_data(){

  this->stationid = "WISCNOAA";
  std::string str;
...
void
NOAAObservation::set_date(const std::string& hour_window1, 
  ...
  this->ndate = stol((org_date + hour_window1 + hour_window2));
}

void
NOAAObservation::time_conversion(){
...
NOAAObservation::uv_conversion(){
  ...
  std::tie(uwnd, vwnd) = noaa_convert_to_uv(spd, dir);
...
std::string
noaa_set_obstype( json & sat, std::string& type ){
...
noaa_retrieve_ids(std::vector<std::string>& sat, std::vector<std::string>& type){
  ...
  for (auto tuple : boost::combine(sat, type)){
    ...
    for(int i = 0; i < sat_list["satellites"].size(); ++i){
      ...
      if(sat.compare(sat_name) == 0){
        subtype.push_back(noaa_set_subtype(sat_list["satellites"][i]));
        obstype.push_back(noaa_set_obstype(sat_list["satellites"][i], type));
        break;
      }
    }
  }
...
void
NOAAObservation::find_numerical_id(){
  ...
  std::tie(subtype, obstype) = noaa_retrieve_ids(sat, type);
...
void
NOAAObservation::lon_conversion(){
...
void
NOAAObservation::do_conversions(){
  time_conversion();
  uv_conversion();
  find_numerical_id();
  lon_conversion();
...
void
NOAAObservation::preprocess(){
  import_file();
}


/* ----------------------------------------------------------------------------
NOAA Observation Non-Member Definitions
---------------------------------------------------------------------------- */

std::tuple<double, double>
noaa_calculate_time(const std::string& old_time){
...
noaa_determine_hour_windows(double start_time, double end_time){
...
std::tuple<double, double>
noaa_find_time_range(const std::vector<std::string>& hms){
...
std::tuple<std::vector<std::string>, std::vector<std::string>>
noaa_convert_to_uv(std::vector<std::string>& spd, std::vector<std::string>& dir){
