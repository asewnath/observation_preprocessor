...
extern "C" {
  ...
  void data_metsat(double (**data_ptr)[10], int ** int_ptr, const char (**date_ptr),
            const char (**file_ptr), int * str_length);
  void get_labels_metsat(char(**str));
}
...
METSATObservation::import_file(){
  ...
  data_metsat(&data_ptr, &int_ptr, &date_ptr, &file_ptr, &str_length);
