...
!-------------------------------------------------------------------------
      ...
      real(r_bfr)    pgm_code            !  'program code' to use for  datazl
...
! !INTERFACE:
      ...
      subroutine init_bufr(outputfile, len_out, tablefile,
     &      len_tab         ) bind(c, name="obs_initbufr")
            
! !INPUT PARAMETERS

      character(:), allocatable :: outfn
      character(:), allocatable :: tablefn

      character(kind=C_CHAR), intent(in) :: outputfile(*)
      character(kind=C_CHAR), intent(in) :: tablefile(*)
      integer(kind=C_INT), intent(in) :: len_out
      integer(kind=C_INT), intent(in) :: len_tab
      integer :: i
 
      !character(len=*, kind=C_CHAR),intent(in) :: tablefile ! BUFR table file
...
!-------------------------------------------------------------------------
      ...
      !print *, length      
      allocate(character(len=len_out)::outfn)
      allocate(character(len=len_tab)::tablefn)
       
      forall(i=1:len_out) outfn(i:i) = outputfile(i)
      forall(i=1:len_tab) tablefn(i:i) = tablefile(i)
...
!      endif
      ...
      !lu = 92
      !print*, "lu: ", lu
      open(unit=lu,file=outfn,form='unformatted',action='write')      
      lu_b = lu
...
! !INTERFACE:
      ...
      subroutine end_bufr() bind(c, name="obs_endbufr")
        
      !character(len=220) cmtdir
! !DESCRIPTION:
!     
