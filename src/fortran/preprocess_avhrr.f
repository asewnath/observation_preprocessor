...
! MODIFIED TO WRITE TO TEXT FILE
      ...
      subroutine get_labels(str_ptr)bind(C, name="get_labels_avhrr")
      use iso_c_binding
     ...
     &      date_ptr,file_ptr,str_length) bind(C, name="data_avhrr")
...
!      call openbf(lunit,'IN',lutbl)
     ...
     .  // 'gdas_prepbufr_2019061212.tbl'
      ...
      num = 0
      print *,ireadmg(lunit, subset, IDATE)
...
!  read all the observations from this BUFR message
     ...
     . 'SAID YEAR MNTH DAYS HOUR MINU SECO SWCM SAZA SCCF CLAT CLON' )
...
!  and the first of the PCCF replication associated with GCLONG and GNAP
            call ufbrep(lunit,qcdat,3,12,ilev3,'OGCE GNAP PCCF')
            qify = 110
            qifn = 110
            rffl = 110
!  check for quality marks based on GCLONG=254 code table for EUMETSAT
            do i = 4,9
               if ( qcdat(2,i) < bmiss .and. qcdat(3,i) < bmiss) then
                  if(qcdat(2,i)==3.0.and.qcdat(3,i)<bmiss) then
                     qify = qcdat(3,i)    ! QI with forecast
                  else if(qcdat(2,i)==1.0.and.qcdat(3,i)<bmiss) then
