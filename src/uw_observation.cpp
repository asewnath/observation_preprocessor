...
UWObservation::set_date(const std::string& hour_window1, 
  ...
  for(auto const& x : this->data_storage){
    if( (x.first.find("hour_window") != std::string::npos)  ){
      hour_window = x.second.front();
    }
  }

  corrected_date = update_date(date, time, hour_window);

  this->ndate = stol((corrected_date + hour_window1 + hour_window2));
